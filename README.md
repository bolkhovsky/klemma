...
**Верификация цитат** — `check-citations` сверяет каждое утверждение со ссылкой против полного текста источника (включая статьи с нумерованными ссылками `[5]`), `claims status --gate` хранит аудит как состояние между сессиями и работает воротами перед подачей, `find-source` находит, какой источник библиотеки действительно поддерживает утверждение. Фрагменты несут честный флаг `дословно/пересказ`.
