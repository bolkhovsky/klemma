...
### extract.md
`title`, `authors`, `year`, `journal`, `doi`, `abstract`, `pdf_text`, `dissertation_context`, `available_tags`, `outline_digest` (optional; `## Project Outline` block, plan C3); `extract_exhaustive.md` — same variables, exhaustive harvest with `notes {contradicts, qualifies}` (plan C4), `chunk_index`/`chunk_total`/`char_start`/`char_end` (chunked mode), `language`, `project_type`, `section_types`
