...
- `test_security_hardening.py` (~130 lines) — 12 tests: vault path traversal (folder/name traversal rejection, valid path allowed), download safety (unsafe URL rejection, large content-length, stream overflow cleanup), `LocalFileStore` path traversal (dotdot/slash in paper_id/filename rejection, empty paper_id, valid ids allowed)
