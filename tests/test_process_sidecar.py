...
def test_partial_extraction_is_kept_out_of_library_cache(tmp_path: Path):
    """Codex P1 on PR-A: a partial first extraction is not published to the
    shared library cache nor registered as completed; the source is degraded."""
    from klemma.literature.models import ExtractionResult, Fragment
    ...
    paper_store.find_paper.return_value = None
    user_library = MagicMock()
    user_library.resolve_paper_id.return_value = None
    partial = ExtractionResult(
        source_id="k", fragments=[Fragment(text="new one")], chunk_total=3, failed_chunks=1,
        coverage_ratio=0.66,
    )
    with (
        patch("klemma.skills.extractor.extract_fragments", return_value=partial),
        patch("klemma.skills.extractor.save_fragments_to_vault", return_value=None),
        patch("klemma.literature.metadata.lookup_s2", return_value=None),
        patch("klemma.hashing.compute_pdf_hash", return_value="h"),
    ):
        n, status = _process_single(
            citekey="k", cfg=cfg, state=state, vault=MagicMock(), ai=MagicMock(),
            pdf_extractor=pdf_extractor, library=library, quiet=True,
            paper_store=paper_store, user_library=user_library,
        )
    assert (n, status) == (1, "ok")
    paper_store.save_fragments.assert_not_called()
    user_library.add_source.assert_not_called()
    steps = state.sources.mark_degraded.call_args.args[1]
