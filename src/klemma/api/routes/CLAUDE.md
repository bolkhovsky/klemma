...
- `GET /library/gaps` → `ReferenceGapsResponse` — papers cited by library sources but not in library. Scored by `count × avg_quality × intent_weight × semantic_factor` (Teufel 2006 taxonomy). Scoring pipeline factored into `api.recommendations.compute_scored_gaps` (shared with `/library/recommendations`). Returns top 10 after `apply_recency_filter` (max_age_years=10; classics with cited_by_count≥3 kept). Requires ≥3 user sources (returns empty + detail otherwise). Response fields: `title, authors, year, doi, cited_by_count, score, avg_quality, intent_weight, semantic_factor, intents: list[str], top_intent: str|null, sections_served: [{section, count}]`. `semantic_factor` is a **noise penalty** [0.5, 1.0], not a boost — cosine=0 halves score.
- `GET /library/recommendations?project_id=<pid>` → `RecommendationsResponse` — LLM-curated top-10 recommendations with 1–2 sentence rationale each (#332). Requires `project_id` (400 if absent) and ownership (404 on another user's project). Pipeline: `compute_scored_gaps(limit=50)` → `select_loaded_sources(max_items=5)` → `build_prompt_inputs` → `ai.call_json(system, user)` → `parse_llm_output`. Cached in `library.db:recommendations_cache` keyed on `(user_id, project_id, library_state_hash, outline_hash, model)`; invalidated on source add/delete and outline change. Fallback on LLM error or empty parse: recency-filtered top-10 with empty rationales + `detail = "AI недоступен — базовая сортировка"` (HTTP 200, partial success). Response: `recommendations: [{title, authors, year, doi, rationale, score ∈ [1,10]}], total, model, generated_at, cached, detail?, warning?`.
- Schemas: `SourceResponse`, `SourceListResponse`, `SourceCreateRequest`, `FragmentResponse`, `SourceDetailResponse`, `UploadResponse`, `FragmentSearchResult`, `FragmentSearchResponse`, `MetadataCurrentFields`, `MetadataPreviewResponse`, `EnrichRequest`, `EnrichResponse`, `SectionServed`, `ReferenceGapResponse`, `ReferenceGapsResponse`, `RecommendationItem`, `RecommendationsResponse`

### admin.py
Admin-only endpoints — mounted with `prefix="/admin"`. Guard: first registered user. All require Bearer auth.
- `POST /admin/backfill/citation-intents?target_user_id=&batch_size=&cursor=` → `BackfillIntentsResponse` — backfill `citation_intent` for a user's citation_graph entries using cached raw_text. Cursor-based pagination: repeat with `next_cursor` until `remaining==0`. Returns `{processed, skipped_no_raw_text, failed, next_cursor, remaining}`. Returns 503 if AI not configured.
