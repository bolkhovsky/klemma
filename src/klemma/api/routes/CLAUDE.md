...
- `GET /library/gaps` → `ReferenceGapsResponse` — papers cited by library sources but not in library. Scored by `count × avg_quality × intent_weight × semantic_factor` (Teufel 2006 taxonomy). Returns top 10 after recency filter (max_age_years=10; classics with cited_by_count≥3 kept). Requires ≥3 user sources (returns empty + detail otherwise). Response fields: `title, authors, year, doi, cited_by_count, score, avg_quality, intent_weight, semantic_factor, intents: list[str], top_intent: str|null, sections_served: [{section, count}]`. `semantic_factor` is a **noise penalty** [0.5, 1.0], not a boost — cosine=0 halves score.
- Schemas: `SourceResponse`, `SourceListResponse`, `SourceCreateRequest`, `FragmentResponse`, `SourceDetailResponse`, `UploadResponse`, `FragmentSearchResult`, `FragmentSearchResponse`, `MetadataCurrentFields`, `MetadataPreviewResponse`, `EnrichRequest`, `EnrichResponse`, `SectionServed`, `ReferenceGapResponse`, `ReferenceGapsResponse`
