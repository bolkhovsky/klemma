...
- `_auto_embed_after_process(citekey, state, embeddings, quiet)` — embeds fragments + recomputes section centroids for a just-processed source; returns `(created, failed)` — a nonzero `failed` marks the source `degraded` (steps in `sources.degraded_steps`); called automatically from `_process_single()` unless `--no-embed` is set, reused by `klemma repair --steps embeddings`
- `_mark_source_degraded(state, citekey, steps)` — marks a source completed-with-defects and prints a visible console warning; `klemma status` shows the degraded counter, `klemma status --degraded` the details, `klemma repair` fixes and clears
...
- `find-source` command (`commands/find.py`): reverse source lookup — embed a claim sentence, rank library sources by best signal (closest fragment via `retrieve_similar_fragments` or source embedding); output: citekey, similarity, `source_locator`/page, fragment preview with verbatim/пересказ mark; warns when the active embedding model covers only part of the corpus; `--json` for scripting. Catches the «верное утверждение под неверной ссылкой» class (plan 2026-08-13 §4.5)
